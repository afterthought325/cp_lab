    ...
    while True:  # Main Menu / Loop through
        ...
        if menu_option == 1:  # prints character information
            ...
            if char[c1].getHP() > char[c2].getHP():
                ...
                print(char[c1].getName(), "has", difference_of_hp,
                      'more hp than', char[c2].getName())
            else:
                difference_of_hp = char[c2].getHP() - char[c1].getHP()
                print(char[c2].getName(), "has", difference_of_hp,
                      'more hp than', char[c1].getName())
            if char[c1].getDamage() > char[c2].getDamage():
                diff_dam = char[c1].getDamage() - char[c2].getDamage()
                print(char[c1].getName(), "has", diff_dam, 'more damage than',
                      char[c2].getName())
            else:
                diff_dam = char[c2].getDamage() - char[c1].getDamage()
                print(char[c2].getName(), "has", diff_dam, 'more damage than',
                      char[c1].getName())
            ...
            newcharacter = '\n' + newcharactername + ',' + str(newcharacterhp) \
                            + ',' + str(newcharacterdamage)
